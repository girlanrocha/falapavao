...
// =========================
// RÁDIO FALA PAVÃO — com reconexão automática
// =========================
let radioUserPaused = false;
let radioShouldPlay = false;
let radioReconnectTimer = null;
let radioReconnectAttempts = 0;
const RADIO_RECONNECT_DELAY = 3000;
const RADIO_MAX_RECONNECT_DELAY = 15000;
...
function scheduleRadioReconnect(reason="Conexão interrompida"){
  if(radioUserPaused || !radioShouldPlay) return;
  clearRadioReconnect();
  radioReconnectAttempts++;
  const delay=Math.min(RADIO_RECONNECT_DELAY * radioReconnectAttempts, RADIO_MAX_RECONNECT_DELAY);
  updateRadioUi(false, `${reason} · reconectando...`);
  radioReconnectTimer=setTimeout(()=>reconnectRadio(),delay);
}
async function reconnectRadio(){
  ...
  if(!audio) return;
  try{
    // Recarrega a URL para abrir uma nova conexão com o ShoutCast.
    ...
    scheduleRadioReconnect("Sinal instável");
  }
}
async function playRadio(){
  ...
  try{
    ...
    updateRadioUi(false,"Toque em ▶ para ouvir ao vivo");
...
function bindRadioRecovery(){
  const audio=getRadio();
  if(!audio || audio.dataset.recoveryBound==="1") return;
  audio.dataset.recoveryBound="1";

  audio.addEventListener("playing",()=>{
    radioReconnectAttempts=0;
    clearRadioReconnect();
    if(radioShouldPlay) updateRadioUi(true);
  });
  audio.addEventListener("waiting",()=>{
    if(radioShouldPlay && !radioUserPaused) updateRadioUi(true,"AO VIVO · carregando sinal...");
  });
  audio.addEventListener("stalled",()=>scheduleRadioReconnect("Sinal interrompido"));
  audio.addEventListener("error",()=>scheduleRadioReconnect("Falha no sinal"));
  audio.addEventListener("ended",()=>scheduleRadioReconnect("Transmissão interrompida"));
  audio.addEventListener("pause",()=>{
    // Se pausou sem comando do usuário, tenta recuperar o stream.
    if(radioShouldPlay && !radioUserPaused) scheduleRadioReconnect("Áudio interrompido");
  });
}
...
window.addEventListener("load",()=>{
  ...
  const unlock=()=>{
    const audio=getRadio();
    if(audio && audio.paused && !radioUserPaused) playRadio();
